...
* Major version number: updated with every new issue. That is, issue #1 was v1.0.0, issue #2 was v2.0.0, etc;
* Minor version number: updated with maintenance releases. These were weekly until issue #4, but after that became rarer, between two to three weeks;
* Patch: only released to fix critical, functionality-breaking errors caused by major and minor updates.
...
* `views`: contains the template html and any necessary partials. Can be as simple as a `.blade` file extending a layout with an empty div or full-blown html with loads of partials and etc;
